    ...
    const CoordinateSystem = (xOrigin: number, yOrigin: number, xWidth: number, yHeight: number, line=true, scale=false) => {
      ...
      const barChart = (barWidth: number, data: any) => {
        data.forEach((it: any,index: number) => {
          ...
          context.beginPath();
          context.moveTo(xOrigin + scaleWidth * (index + 1) + barWidth / 2, yOrigin - barWidth / 2 - lineWidth);
          context.arc(xOrigin + scaleWidth * (index + 1), yOrigin - barWidth / 2 - lineWidth, barWidth / 2, 0, Math.PI, false);
          context.moveTo(xOrigin + scaleWidth * (index + 1) + barWidth / 2, yOrigin - h - barWidth / 2 - lineWidth);
          context.arc(xOrigin + scaleWidth * (index + 1), yOrigin - h - barWidth / 2 - lineWidth, barWidth / 2, 0, Math.PI, true);

          context.rect(xOrigin + scaleWidth * (index + 1) - barWidth / 2, yOrigin - h - barWidth / 2 - lineWidth, barWidth, h);
          const linearGradient = context.createLinearGradient(xOrigin + scaleWidth * (index + 1) - lineWidth, yOrigin, xOrigin + scaleWidth * (index + 1), yOrigin-h-barWidth);
